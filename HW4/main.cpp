...
void test_it(const Size & size)
{
	...
	int carCount1 = 0, carCount2 = 0, carCount3 = 0;
	string s1, s2, s3;
	...
	while (!end_of_process)
	{
		...
		//cuda::GpuMat myImg;
		//myImg.upload(img);
		img = Mat(img, Rect(0,360/2,640/2,360/2));
		my_hog.detectMultiScale(img, locations, 0);
		...
		for (int i = 0; i < locations.size(); ++i){
			...
			if (x > 30 && x < 155 && y > 229 && y < 231)
			{
				carCount1++;
				s1 = to_string(carCount1);
			}
			if (x > 154 && x < 220 && y > 229 && y < 231)
			{
				carCount2++;
				s2 = to_string(carCount2);
			}
			if (x > 219 && x < 300 && y > 229 && y < 231)
			{
				carCount3++;
				s3 = to_string(carCount3);
			}
		}
...
//		resize(draw,draw,Size(1280,720));

		//Display
		putText(draw, "Line 1 - Number Of Cars: " + s1, Point(10, 10), 1, 1, Scalar(105, 50, 205), 1);
		putText(draw, "Line 2 - Number Of Cars: " + s2, Point(10, 30), 1, 1, Scalar(105, 50, 205), 1);
		putText(draw, "Line 3 - Number Of Cars: " + s3, Point(10, 50), 1, 1, Scalar(105, 50, 205), 1);

		line(draw, Point(60, 230), Point(300, 230), Scalar(100, 255, 255), 3);
		circle(draw, Point(60, 230), 3, Scalar(255, 0, 0), 3);
		circle(draw, Point(155, 230), 3, Scalar(255, 0, 0), 3);
		circle(draw, Point(220, 230), 3, Scalar(255, 0, 0), 3);
		circle(draw, Point(300, 230), 3, Scalar(255, 0, 0), 3);
		imshow("Video", draw);
		key = (char)waitKey(1);
		if (27 == key)
			end_of_process = true;
...
/*
	...
	load_images("cars/", "../cars.lst", pos_lst);
	labels.assign(pos_lst.size(), +1);
	const unsigned int old = (unsigned int)labels.size();
	load_images("dump/", "../negative.lst", full_neg_lst);
	//sample_neg(full_neg_lst, full_neg_lst, SIZE);
	labels.insert(labels.end(), full_neg_lst.size(), -1);
	CV_Assert(old < labels.size());

	compute_hog(pos_lst, gradient_lst, SIZE);
	compute_hog(full_neg_lst, gradient_lst, SIZE);

	train_svm(gradient_lst, labels);
*/
	test_it(SIZE); // change with your parameters
